...
function POS({
  ...
  async function fetchOrder() {
    try {
      const res = await axios.get(`${apiURL}/api/generate_order`);
      const data = res.data;
      setOrder(data);
    } catch (error) {
  ...
  function handleDeleteItemClick(currentItem, items) {
    ...
    Object.keys(items).forEach((menu) => {
      ...
      const itemIndex = items[menu].findIndex(
        (menuItem) => menuItem === currentItem
      );
    ...
    <>
      <div className="flex" id="pos-div">
        <Screen
          ...
          order={order}
          handleScreenItemClick={handleScreenItemClick}
        />
        ...
        <div id="pos-buttons-div">
          ...
          <div id="control-buttons" className="flex-buttons">
            ...
            <CloseCheck
              ...
              setCheckIsClosed={setCheckIsClosed}
